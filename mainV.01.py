...
def main():
    ...
    for page_num in range(len(pdf_document)):
        ...
        if any(term in page_text for term in ['crack']):
            filtered_data_xlsx['Page Number'].append(page_num + 1)
            filtered_data_xlsx['Crack Description'].append(page.get_text())
    ...
    filtered_data_df['Crack Description'] = filtered_data_df['Crack Description'].apply(lambda x: re.sub(r'\s+', ' ', str(x)).strip())

    wb = Workbook()
    ws1 = wb.active
    ws1.title = 'Filtered Data'
    
    ws1.append(['Page Number', 'Crack Description', 'Image Reference'])

    row_num = 2

    for page_num, full_text, image_refs in zip(filtered_data_xlsx['Page Number'], filtered_data_xlsx['Crack Description'], filtered_data_xlsx['Image Reference']):
        ...
        if image_refs:
            for img_index, img_file in enumerate(image_refs.split(', ')):
                ...
                if os.path.exists(img_path):
                    ...
                    ws1.column_dimensions[chr(64 + col_num)].width = 30
                    ws1.row_dimensions[row_num].height = 130
    ...
    # Adjust row height
    ws1.row_dimensions[1].height = 80  # Increased to better demonstrate text wrapping

    # Align text and enable text wrapping
    alignment = Alignment(horizontal='left', vertical='top', wrap_text=False)
    
    ws1['B1'].alignment = alignment
    ################################
